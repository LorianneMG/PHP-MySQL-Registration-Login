...
function display_record_table($records)
{
    ...
    foreach ($records as $row) {
        ...
        $grade = $row->gpa;
        if ($grade <= 2.0) {
            echo "<td class='text-center bg-danger text-white'>{$row->gpa}</td>";
        } elseif ($grade == 4.0) {
            echo "<td class='text-center bg-success text-white'>{$row->gpa}</td>";
        } else {
            echo "<td class='text-center'>{$row->gpa}</td>";
        }
